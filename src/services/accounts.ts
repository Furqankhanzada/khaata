...
export const accountInput = z.object({
  ...
  currency: currencyCode.optional().describe("Account currency (defaults to the household base), e.g. 'USD' for Payoneer/Upwork"),
...
export async function listAccounts(ctx: Ctx) {
  ...
  const { rows } = await db.execute(sql`
    select a.*, a.balance::float8 as balance,
           (case when a.currency = ${ctx.baseCurrency} then 1 else fx.rate end)::float8 as rate,
           fx.as_of::text as rate_as_of,
           round(a.balance * (case when a.currency = ${ctx.baseCurrency} then 1 else fx.rate end), 2)::float8 as base_balance
    from accounts a
    left join lateral (
      select rate, as_of from fx_rates where base = ${ctx.baseCurrency} and quote = a.currency
...
export async function addAccount(ctx: Ctx, input: z.infer<typeof accountInput>) {
  ...
  if (currency !== ctx.baseCurrency) await latestRate(ctx.baseCurrency, currency).catch(() => {})
...
export async function updateAccount(ctx: Ctx, id: string, input: z.infer<typeof accountUpdate>) {
  if (input.currency && input.currency !== ctx.baseCurrency) await latestRate(ctx.baseCurrency, input.currency).catch(() => {})
